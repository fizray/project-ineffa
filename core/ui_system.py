...
class UISystem:
    """
    Handles the drawing of the user interface overlay on the video frames.
    Includes dashboard elements like header, sidebar logs, and bounding boxes.
    """
    def __init__(self, config):
        ...
        Args:
            config (dict): The configuration dictionary containing UI settings.
        """
        self.config = config
    ...
    def draw_dashboard(self, frame, tracked_faces, faces_map, recent_logs, is_connected=True):
        """
        Draws the complete dashboard on the frame.

        Args:
            frame (numpy.ndarray): The video frame to draw on.
            tracked_faces (dict): Dictionary of tracked face states.
            faces_map (dict): Dictionary mapping object IDs to face objects.
            recent_logs (list): List of recent attendance logs to display.
            is_connected (bool): Status of the camera connection.

        Returns:
            numpy.ndarray: The frame with the UI overlay.
        """
        self.update_fps()
        h, w = frame.shape[:2]
    ...
    def _draw_header(self, frame, width, is_connected):
        """
        Draws the top header bar with system title and status.
        """
    ...
    def _draw_sidebar(self, frame, height, width, recent_logs):
        """
        Draws the right sidebar showing recent attendance logs.
        """
        sidebar_w = 300
        overlay = frame.copy()
    ...
    def _draw_tracking(self, frame, tracked_faces, faces_map):
        """
        Draws tracking information (bounding boxes, names, scores) on detected faces.
        """
    ...
    def _draw_corners(self, frame, bbox, color, length=20, thickness=2):
        """
        Draws stylized corners for the bounding box.
        """
        x1, y1, x2, y2 = bbox
        # Top Left
